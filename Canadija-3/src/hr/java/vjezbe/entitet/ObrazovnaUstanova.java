...
import hr.java.vjezbe.iznimke.NeispravanNazivPredmeta;
	...
	public static ObrazovnaUstanova unosObrazovneUstanove(Scanner scanner) {
		...
		for (int i = 0; i < BROJ_PREDMETA; i++) {
			do {
				...
				} catch (NeispravanNazivPredmeta e) {
					log.error("Neispravan naziv predmeta: " + e.getLocalizedMessage());
					System.out.println("Neispravan naziv predmeta");
					predmeti[i] = null;
				}
			} while (predmeti[i] == null);
		}
		...
		for (int i = 0; i < BROJ_ISPITNIH_ROKOVA; i++) {
			do {
				...
				} catch (NeispravnaOcjena e) {
					log.error("Neispravna ocjena: " + e.getLocalizedMessage());
					ispiti[i] = null;
				}				
			} while (ispiti[i] == null);
		}
